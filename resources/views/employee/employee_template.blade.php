...
@include('employee.css.employee_css')
@include('employee.js.employee_controls')
@include('employee.js.employee_databinding')
...
<script type="text/javascript">
    ...
    function fn_download_empinfo_listing(){
		...
		var	lv_table = ui('EMPINFO_LISTING_TABLE');
		var lt_content = lv_table.getModel().getData();
		
		var lt_columns = ui('EMPINFO_LISTING_TABLE').getColumns();
		var lt_temp = [];
		...
		for(var i = 0; i < lt_content.length; i++){
			...
			for(var x=0; x<lt_columns.length; x++){
				...
				lv_key = ui('EMPINFO_LISTING_TABLE').getColumns()[x].getLabel().getText();
				lv_field = ui('EMPINFO_LISTING_TABLE').getColumns()[x].getFilterProperty();
		...
		fileName = "Employee Information Listing".replace(/ /g,"_") + downloadedDate;
